...
class TxIn:
    ...
    def validateTxIn(txIn, transaction, unspentTxOuts):
        '''Validate a TxIn object.

        Argument:
            txIn: the TxIn object to validate.

            transaction: the Transaction object of which the TxIn is a part.

            unspentTxOuts: unspect TxOut objects that the TxIn object comes from.

        Returns:
            True/False stating whether the TxIn object is valid or not.
        '''
...
class Transaction:
    ...
    def getTransactionId(self):
        '''Generates and returns the transaction ID which is a function of the transaction contents.'''
        txInContent = ''
        for i in self.txIns:
            txInContent += i.txOutId + str(i.txOutIndex)
        
        txOutContent = ''
        for i in self.txOuts:
    ...
    def validateBlockTransaction(aTransactions, unspentTxOuts, blockIndex):
        ...
        return(reduce(lambda x,y:x and y, list(map(lambda z: validateTransaction(z,unspentTxOuts), transactions)),True))
    ...
    def updateUnspentTxOuts(aTransactions, unspentTxOuts):
        '''
        print("\nunspentTxOuts.............")
    ...
    def processTransactions(aTransactions, unspentTxOuts, blockIndex):
        if(not validateBlockTransaction(aTransactions, unspentTxOuts, blockIndex)):
            logger.error("Invalid Block Transaction")
            return(None)
        
        return(updateUnspentTxOuts(aTransactions, unspentTxOuts))
